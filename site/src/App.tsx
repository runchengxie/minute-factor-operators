...
export default function App() {
  ...
  return <><header className="topbar"><a className="brand" href={import.meta.env.BASE_URL}>MINUTE FACTOR <span>OBSERVATORY</span></a><nav><a href={`${import.meta.env.BASE_URL}jumps`}>跳跃分解</a><a href={`${import.meta.env.BASE_URL}hermite`}>Hermite 体制</a><a href={`${import.meta.env.BASE_URL}fundamentals`}>基本面因子</a></nav></header>{pathView(snapshot, fundamental.catalog, fundamental.snapshot)}<footer>minute-factor-operators · {snapshot.source === 'demo' ? 'Demo snapshot' : 'Parquet snapshot'} · 研究展示，不构成交易建议</footer></>
}
